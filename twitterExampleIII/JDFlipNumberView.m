...
- (void) initImages
{
	...
	for (int i = 0; i < 2; i++)
	{
		for (NSString* filename in filenames)
		{
			...
			CGSize size		= CGSizeMake(scaledImage.size.width, scaledImage.size.height);
			...
			[scaledImage drawAtPoint:CGPointMake(0.0,yPoint)];
...
- (void) setIntValue: (NSUInteger) newValue
{
	...
	// inform delegate
	if ([_delegate respondsToSelector: @selector(flipNumberView:didChangeValue:animated:)]) {
		[_delegate flipNumberView: self didChangeValue: mCurrentValue animated: NO];
	}
}
...
- (void) animateIntoCurrentDirectionWithDuration: (CGFloat) duration
{
	...
	if (duration < 0.05) {
		// inform delegate
		if ([_delegate respondsToSelector: @selector(flipNumberView:willChangeToValue:)]) {
			[_delegate flipNumberView: self willChangeToValue: nextIndex];
		}
...
- (void)animationDidStop:(CAAnimation *)theAnimation finished:(BOOL)flag
{
	...
	else
	{
		...
		// inform delegate
		if ([_delegate respondsToSelector: @selector(flipNumberView:didChangeValue:animated:)]) {
			[_delegate flipNumberView: self didChangeValue: mCurrentValue animated: YES];
		}
	}
}
...
{	
	...
	[self stopAnimation];
	mTimer = [NSTimer scheduledTimerWithTimeInterval: timeInterval target: self selector: @selector(animateToNextNumber) userInfo: nil repeats: YES];
}
...
{	
	...
	[self stopAnimation];
	mTimer = [NSTimer scheduledTimerWithTimeInterval: timeInterval target: self selector: @selector(animateToPreviousNumber) userInfo: nil repeats: YES] ;
}
