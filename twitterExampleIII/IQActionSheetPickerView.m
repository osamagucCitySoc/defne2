...
- (id)init
{
    ...
    if (self) {
        ...
        if([[NSUserDefaults standardUserDefaults]objectForKey:@"MAXDATE"])
        {
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
            [dateFormatter setDateFormat:@"dd/MM/yyyy HH:mm"];
            NSDate* maxDate = [dateFormatter dateFromString:[[NSUserDefaults standardUserDefaults]objectForKey:@"MAXDATE"]];
            [_datePicker setMinimumDate:maxDate];
            NSLog(@"min: %@, max: %@",_datePicker.minimumDate,_datePicker.maximumDate);

            [[NSUserDefaults standardUserDefaults]setObject:nil forKey:@"MAXDATE"];
...
-(void)setMin:(NSDate*)date
{
    [_datePicker setMinimumDate:date];
}

-(void)setMax:(NSDate*)date
{
...
-(void)setActionSheetPickerStyle:(IQActionSheetPickerStyle)actionSheetPickerStyle
{
    ...
    switch (actionSheetPickerStyle) {
        ...
        case IQActionSheetPickerStyleDatePicker:
            ...
            break;
        case IQActionSheetPickerStyleDatePicker2:
            [_datePicker setDatePickerMode:UIDatePickerModeDate];
            [_pickerView setHidden:YES];
            [_datePicker setHidden:NO];
            break;
        case IQActionSheetPickerStyleDatePicker3:
            [self minDate];
            [_pickerView setHidden:YES];
            [_datePicker setHidden:NO];
            [[NSUserDefaults standardUserDefaults]setObject:nil forKey:@"MAXDATE"];
...
-(void)minDate
{
    ...
    [dateFormatter setDateFormat:@"dd/MM/yyyy HH:mm"];
    NSDate* maxDate = [dateFormatter dateFromString:[[NSUserDefaults standardUserDefaults]objectForKey:@"MAXDATE"]];
    maxDate = [maxDate dateByAddingTimeInterval:60];
    [_datePicker setMinimumDate:maxDate];
    [[NSUserDefaults standardUserDefaults]setObject:nil forKey:@"MAXDATE"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    }
...
-(void)pickerDoneClicked:(UIBarButtonItem*)barButton
{
    if ([self.delegate respondsToSelector:@selector(actionSheetPickerView:didSelectTitles:)])
    {
        ...
        else if (_actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker || _actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker3)
        {
            ...
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
            [dateFormatter setDateFormat:@"dd/MM/yyyy HH:mm"];
            NSString *formattedDateString = [dateFormatter stringFromDate:_datePicker.date];
            [self setSelectedTitles:[[NSArray alloc] initWithObjects:_datePicker.date, nil]];
            selectedTitles = [[NSMutableArray alloc]initWithObjects:formattedDateString, nil];
        }else if(_actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker2)
        {
            [selectedTitles addObject:[NSDateFormatter localizedStringFromDate:_datePicker.date dateStyle:_dateStyle timeStyle:NSDateFormatterNoStyle]];
            [self setDate:_datePicker.date];
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
...
-(NSArray *)selectedTitles
{
    ...
    else if (_actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker || _actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker2 || _actionSheetPickerStyle == IQActionSheetPickerStyleDatePicker3)
    {
